...
function getStateNodes() {
  return document.getElementById('locations-list').querySelectorAll('section[data-state]');
}

function getCityNodes() {
  return document.getElementById('locations-list').querySelectorAll('section[data-city]');
}

function getLocationNodes() {
  return document.getElementById('locations-list').querySelectorAll('article[data-location]');
}

function extractDataByLocation() {
  // Assume states are already in sorted order.
  const data_by_location = {};

  for (const state_node of getStateNodes()) {
    const state = state_node.dataset['state'];
    const state_entry = data_by_location[state] = {};

    for (const city_node of state_node.querySelectorAll('article[data-city]')) {
      const city = city_node.dataset['city'];
      state_entry[city] = [];

      for (const entry_node of city_node.querySelectorAll('article[data-entry]')) {
        // Hack to turn DOMStringMap into a normal object.
        state_entry.push(JSON.parse(JSON.stringify(entry_node.dataset)));
      }
    }
  }
...
function hideShowLocations(checked_states, accept_items) {
  const all_state_nodes = getStateNodes();
  const all_city_nodes = getCityNodes();
  const all_location_nodes = getLocationNodes();

  // Hide everything to start.
  all_state_nodes.forEach(l => l.classList.add('hidden'));
  all_city_nodes.forEach(l => l.classList.add('hidden'));
  all_location_nodes.forEach(l => l.classList.add('hidden'));

  const locations = [];
  if (checked_states.length > 0) {
  console.log(checked_states);
    for (const state of [...all_state_nodes].filter(n => checked_states.includes(n.dataset['state']))) {
  console.log(checked_states);
      locations.push(...state.querySelectorAll('article[data-location]'));
    }
  } else {
    locations.push(...getLocationNodes());
  }

  for (const l of locations) {
    if (accept_items.length > 0) {
      const accepting = l.dataset['accepting'].toLowerCase();
      if (accept_items.some(s => accepting.includes(s))) {  
  ...
  // Now all locations are correctly marked hidden. Walk up the hierarchy
  // and hide/show the city and states.
  for (const city of all_city_nodes) {
    if (city.querySelector('article[data-location]:not(.hidden)')) {
    console.log(city);
      city.classList.remove('hidden');
    }
  }

  for (const state of all_state_nodes) {
    if (state.querySelector('section[data-city]:not(.hidden)')) {
...
document.addEventListener("DOMContentLoaded", function() {
  window.data_by_location = extractDataByLocation();
  ...
  if (!hideList) {
    ...
    if (!searchParams.get('hide-filters') || searchParams.get('hide-filters') !== 'true') {
      document.getElementsByClassName('filters-container')[0].classList.remove('hidden');
    }
...
function onFilterChange(elem, scrollNeeded) {
  ...
  const checkedStates = [...document.filters['states']].filter(s => s.checked).map(s => s.value);
...
//  });
  ...
  const acceptItems = [...document.filters['accept-item']].filter(item => item.checked).map(item => item.value);

  hideShowLocations(checkedStates, acceptItems);
  if (scrollNeeded) {
...
function initMap(states) {
     ...
     document.getElementsByClassName('map-container')[0].classList.remove('hidden');
